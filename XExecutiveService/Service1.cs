...
namespace XTRMExecutiveService
{
    public partial class Service1 : ServiceBase
    {
        ...
        public Service1()
        {
            //this.ServiceName = "XExecutive XBot Service";
            InitializeComponent();

            if (!System.Diagnostics.EventLog.SourceExists("XExecutive"))
            {
                System.Diagnostics.EventLog.CreateEventSource(
                    "XExecutive", "Application");
            }
            eventLog1.Source = "XExecutive";
            eventLog1.Log = "Application";

            try
            {
                myExecutiveCore = new XTRMExecutive(eventLog1);
            }
            catch (Exception ex)
            {
                string tempstr = string.Format("{0} Exception in XExecutiveCore() Constructor; Message={1}", AssemblyInfo.buildEnv, ex.Message);
        ...
        protected override void OnStop()
        {
            ...
            myExecutiveCore.XLogger(0, string.Format("Stopping...{0} Version={1} Built={2}.", AssemblyInfo.buildEnv, AssemblyInfo.buildVersion, AssemblyInfo.buildTime), 9903);
            ...
            while (bProcessing & (count < 20))
            {
                ...
                eventLog1.WriteEntry(tempstr);
                myExecutiveCore.XLogger(0, tempstr, 9904);
                Thread.Sleep(1000);
            }
            if (bProcessing)
            {
                ...
                eventLog1.WriteEntry(tempstr);
                myExecutiveCore.XLogger(0, tempstr, 9905);
        ...
        protected void OnTimedEvent(object source, ElapsedEventArgs e)
        {
                ...
                if (!bQuiesce)
                {
                    if (!bProcessing)
                    {
                        ...
                        for (int i = 0; i < 2; i++)
                        {
                            ...
                            thisTimeInner.Start();
                            //int i = 1;
                            //if (myExecutiveCore.Run(1, logBeat) >= 0)
                            //{
                            //    i = 0;
                            //    myExecutiveCore.Run(0);
                            //}
                            if (myExecutiveCore.Run(i, false) < 0)
                            {
                                break;
                            }
                            thisTimeInner.Stop();
            ...
            catch (Exception ex)
            {
                ...
                myExecutiveCore.XLogger(0, tempstr, 9906);
        ...
        protected int checkParms()
        {
            ...
            xBotID = XTRMObject.getXTRMID();
            int interval = Convert.ToInt32(XTRMObject.getDictionaryEntry("ExecutiveInterval", "10"));
            ...
            if (!aTimer.Interval.Equals(interval * 1000))
            {
                ...
                myExecutiveCore.XLogger(0, tempstr, 9907);
                eventLog1.WriteEntry(tempstr);
            ...
            if (pendingStop == 1)
            {
                ...
                eventLog1.WriteEntry(tempstr);
                myExecutiveCore.XLogger(0, tempstr, 9913);
                rc = -5;
            }
            ...
            else if (Convert.ToInt32(serviceLife / 1000) >= restartAfterSeconds)
            {
                ...
                myExecutiveCore.XLogger(0, tempstr, 9908);
                eventLog1.WriteEntry(tempstr);
            ...
            else if (memorySize > restartOverMemory)
            {
                ...
                myExecutiveCore.XLogger(0, tempstr, 9909);
                eventLog1.WriteEntry(tempstr);
        ...
        protected void heartBeat()
        {
            ...
            myExecutiveCore.XLogger(0, tempstr, 9999);
            eventLog1.WriteEntry(tempstr);
            if (XTRMObject.getDictionaryEntry("ShowTimingSummary", "Y").Equals("Y"))
            {
                ...
                myExecutiveCore.XLogger(0, tempstr, 9979);
                eventLog1.WriteEntry(tempstr);
            }
            if (XTRMObject.getDictionaryEntry("ShowTimingDetail", "Y").Equals("Y"))
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    myExecutiveCore.XLogger(0, tempstr, 9980 + i);
                    eventLog1.WriteEntry(tempstr);
                }
            }
            if (!XTRMObject.getDictionaryEntry("ProcessJobs", "Y").Equals("Y"))
            {
                ...
                myExecutiveCore.XLogger(0, tempstr, 9910);
                eventLog1.WriteEntry(tempstr);
            }
            return;
        }
